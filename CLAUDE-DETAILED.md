# 📚 [踩坑指南] CLAUDE-DETAILED.md

> **用途**：環境事實、已知踩坑、品質標準（QB 清單）、解決方案
> **讀者**：遇到具體問題時查閱（不是開工必讀）
> **更新頻率**：每季更新一次（新踩坑記錄）
> **相關檔案**：← CLAUDE.md（規則）、→ math-deliverable-qa skill（QA 工具）
...
- 🔴 **文件最後一個元素是表格 → Word 自動補一個看不見的空段落，可能迫出整版空白頁**
  （2026-09-20 實測）：`build_docx()` 不會補尾段，但 Word 開檔時會自動補一個 Normal 樣式的
  空段落（12pt ＋ 段後 8pt ≈ 24pt）。最後一個題框剛好貼住頁底時，這個看不見的段落就足以
  多迫一頁**完全空白**的第 2 頁（PyMuPDF 抽字會顯示該頁無內容，肉眼看 PDF 也只見空白）。
  → 自己在最後補一個 1pt 高的空段落（`w:sz="2"`、`w:line="100"`、before/after 0），
  Word 就不會再補，省回約 22pt。範例見 `額外評核\quiz_common.py` 的 `tail_stub()`。
- 表格儲存格塞純字串 list → Word 報毀損。→ `_tc()` 已加自動包 `para()` 的防呆。
- 表格跨頁無表頭 → 第二頁看不懂。→ 表頭列加 `hdr: True`。
- 🔴 **連續同款 `w:pBdr` 底框段落，Word 只畫一條線**（2026-09-20 實測揪出，影響所有舊交付檔）：
  Word 把「連續、邊框設定完全相同」的段落當成一個 group，只在 group 最頂畫 `w:top`、
  最底畫 `w:bottom`，**中間那幾條完全不畫**。所以 `write_lines(5)` 在 PDF 上只剩 1 條線；
  小測的「作答：」「改正：」兩段孖住擺，「作答：」下面那條也會消失。
  → 修法：`w:pBdr` 內加 `<w:between>`（同款式），段與段之間那條就會畫出來。
  CT_PBdr 的 schema 次序是 `top, left, bottom, right, between, bar`，
  `w:between` **必須排在 `w:bottom` 後面**，掉轉 Word 會報檔案損毀。
  已修：`_shared-math-docx` 下游的 `omml_docx.write_lines()`、額外評核的 `quiz_common.ruled()`。
  🔴 **連環坑一**：`w:between` 的 `w:space` 會**逐條加到每行高度**。跟 `w:bottom` 一樣用 `w:space="4"`
  會把單頁講義撐成兩頁。**between 一律用 `w:space="0"`**，`w:bottom` 的維持原值。
  🔴 **連環坑二**：即使 between 用 0，本來沒畫出來的線現在畫出來了，整體仍會略高。
  收版面的次序（記憶 `table_doc_page_saving_order`）：① 框內段落的段後 8pt→0
  ② 書寫線的段前 60twips→40 ③ 框間分隔段縮小。
  **行距 1.5 倍（`w:line="360"`）不准動**——house-style 第 17 行明訂，且是讀寫障礙友善的硬需求。
  ⚠️ **2026-09-20 之前產出的講義／練習 PDF 全部帶這個缺陷**，作答線數目都不對，重出才會修好。
...
| QB-6 | 檔名慣例 | 交付檔 | 前綴 ∈ {講義,練習,功課,複習,測驗,公式紙,簡報,工具卡,整合教材,教學計劃,IEP,**額外評核,抽離課堂檢測,課後補救教學**} |
| QB-7 | 黑白 | docx（IEP 除外） | document.xml 無彩色碼 |
| QB-8 | 學生資訊列 | 講義/練習/功課/複習/測驗/三類評核 PDF | 第 1 頁含 姓名、班別、學號；**分卷型的 `_教師卷` 自動 SKIP** |
| QB-9 | 星星三層 | 融合／抽離小班練習 PDF | 全文含 ★☆☆、★★☆、★★★ |
| QB-10 | 題答對應 | 練習/功課/測驗/三類評核 PDF | 題號集合相等且非空；**分卷型改為跨檔比對**（`_學生卷` 題號 vs `_教師卷` 答案區題號） |
| QB-11 | 驗算紀錄 | 練習/功課/測驗/三類評核 | `驗算*.md` 的題號 ⊇ 文件題號。題號兩種格式都認：正文「1．」與 Markdown 表格「\| 1 \|」 |
...
**分卷型交付（2026-09-20 立）** — 題目與答案刻意分兩個檔（`_學生卷`／`_教師卷`，或 `_標準版`／`_輔助版`／`_教師卷`）。
只把新前綴加入 QB-6 白名單解決不到問題：教師卷沒有學生資訊列 → QB-8 必 FAIL；
學生卷內找不到「參考答案」節 → QB-10 必 FAIL，兩個都是假陽性。
`qa_check.py` 的 `split_role()`／`mate_of()` 會認出卷別：教師卷的 QB-8／QB-11 改 SKIP、
QB-10 只查它有沒有答案節；題目卷的 QB-10 則去配對的教師卷抓答案區題號做跨檔比對。
**要點：兩個檔要放同一資料夾、檔名只差最後的卷別字樣**，否則配對不到會 FAIL。

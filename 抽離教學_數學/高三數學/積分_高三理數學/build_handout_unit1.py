...
P.append(para('① 先看清楚這是幾項相加減——三項，逐項標號：{∫(①6x^2②−4x③+3) dx}'))
P.append(para('② 用和差法則拆開，每項再用常數倍法則把係數提出來：{①6∫x^2 dx②−4∫x dx③+3∫1 dx}'))
P.append(para('③ 對每一項套用公式卡的冪函數公式（第2條）逐項計算：{①6*frac(x^3,3)②−4*frac(x^2,2)③+3x}'))
P.append(para('④ 化簡每一項：{①2x^3②−2x^2③+3x}'))
P.append(para('⑤ 所有項合併，最後只加「一個」C（不是每項各加一個C）：{2x^3−2x^2+3x+C}'))
P.append(blank())

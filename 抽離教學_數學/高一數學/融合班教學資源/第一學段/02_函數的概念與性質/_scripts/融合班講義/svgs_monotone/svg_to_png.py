...
def render(svg_path, png_path, w, h, scale=3):
    ...
    # Chrome on this box logs a page-file/DLL warning and returns non-zero even
    # when the screenshot writes successfully -- check the output file, not the exit code.
...
SIZES = {
    'diagram1_monotone_intro.svg': (340, 240),
    'diagram2_parity_intro.svg': (520, 220),
    'diagram3_practiceA_graph.svg': (380, 320),
}
...
if __name__ == '__main__':
    ...
    for svg_path in glob.glob(os.path.join(indir, '*.svg')):
        ...
        w, h = SIZES.get(name, (400, 300))
        png_path = os.path.join(indir, name.replace('.svg', '.png'))
        render(svg_path, png_path, w, h)
